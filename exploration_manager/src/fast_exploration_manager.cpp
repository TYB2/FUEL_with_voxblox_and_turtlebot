...
int FastExplorationManager::planExploreMotion(
    Vector3d pos, const Vector3d& vel, const Vector3d& acc, const Vector3d& yaw) {
  ...
  double frontier_time = (ros::Time::now() - t1).toSec();  // time of searching frontiers. useless
  t1 = ros::Time::now();
  ...
  frontier_finder_->getDormantFrontiers(ed_->dead_frontiers_);  // Dormant Frontiers are useless?
  ...
  double view_time = (ros::Time::now() - t1).toSec();  // useless
  ROS_WARN(
  ...
  if (ed_->points_.size() > 1) {
    ...
    std::cout << "size of ed_->points_ is more than 1" << std::endl;
    vector<int> indices;
    findGlobalTour(pos, vel, yaw, indices);

    if (ep_->refine_local_) {  // it is true
      ...
      // what are refined_views1_ and refined_views2_ ? 
    ...
    std::cout << "size of ed_->points_ is 1" << std::endl;
  ...
  double diff = fabs(next_yaw - yaw[0]); // difference of yaw
  ...
  ROS_ERROR_COND(total > 0.1, "Total time too long!!!");  // the time of planning is too long

  return SUCCEED;
}

int FastExplorationManager::planExploreMotion(
    Vector3d pos, const Vector3d& vel, const Vector3d& acc, const Vector3d& yaw, Vector3d& next_pos, double& next_yaw) {
  ros::Time t1 = ros::Time::now();
  auto t2 = t1;
  ed_->views_.clear();
  ed_->global_tour_.clear();
  ...
  // Find viewpoints (x,y,z,yaw) for all frontier clusters and get visible ones' info
  frontier_finder_->computeFrontiersToVisit();
  // double computeFrontiersToVisit_time = (ros::Time::now() - t1).toSec();
  frontier_finder_->getFrontiers(ed_->frontiers_);
  // double getFrontiers_time = (ros::Time::now() - t1).toSec();
  frontier_finder_->getFrontierBoxes(ed_->frontier_boxes_);
  // double getFrontierBoxes_time = (ros::Time::now() - t1).toSec();
  frontier_finder_->getDormantFrontiers(ed_->dead_frontiers_);  // Dormant Frontiers are useless?
  // double getDormantFrontiers_time = (ros::Time::now() - t1).toSec();
  ...
  ROS_WARN(
      ...
      ed_->points_.size(), view_time);
  // ROS_WARN(
  //     "computeFrontiersToVisit t: %lf, getFrontiers t: %lf, getFrontierBoxes t: %lf, getDormantFrontiers t: %lf", 
  //     computeFrontiersToVisit_time, getFrontiers_time, getFrontierBoxes_time, getDormantFrontiers_time);
...
void FastExplorationManager::findGlobalTour(
  ...
  double mat_time = (ros::Time::now() - t1).toSec(); // useless
  t1 = ros::Time::now();
  ...
  } else {   // only execute following codes
    // Use Asymmetric TSP
...
void FastExplorationManager::refineLocalTour(
  ...
  search_time = (ros::Time::now() - t1).toSec(); // useless
  t1 = ros::Time::now();
