...
void SDFMap::inputPointCloud(
    ...
    const Eigen::Vector3d& camera_pos) {    // 输入点云，点云中点的数量和当前相机位置坐标
  ...
  for (int i = 0; i < point_num; ++i) {
    ...
    posToIndex(pt_w, idx); // 点对应的index
    vox_adr = toAddress(idx); // index对应的地址
    setCacheOccupancy(vox_adr, tmp_flag); // 没看懂这个函数是干什么用的，主要是这个函数里面的两个变量我没看懂是做什么用的
    ...
    if (md_->flag_rayend_[vox_adr] == md_->raycast_num_)
      ...
      md_->flag_rayend_[vox_adr] = md_->raycast_num_;  // 没看懂
