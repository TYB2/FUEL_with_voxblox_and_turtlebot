...
public:
  ...
  void getDebugFrontiers(vector<vector<Eigen::Vector3d>>& clusters);
  void getDebugSamplePoints(vector<vector<Eigen::Vector3d>>& clusters);
...
private:
  ...
  vector<Eigen::Vector3i> fourNeighbors2D(const Eigen::Vector3i& voxel);
  ...
  vector<Eigen::Vector3i> allNeighbors2D(const Eigen::Vector3i& voxel);
  ...
  unordered_map<int, bool> frontier_flag_;
  // map<int, int> frontier_flag_;
  list<Frontier> frontiers_, dormant_frontiers_, tmp_frontiers_, debug_frontiers_;
  vector<int> removed_ids_;
  list<Frontier>::iterator first_new_ftr_;
  Frontier next_frontier_;
  vector<vector<Vector3d>> debugSamplePoints;
