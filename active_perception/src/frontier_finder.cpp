...
void FrontierFinder::searchFrontiers() {
  std::cout << "FrontierFinder start-------------------------" << std::endl;
  ...
  for (auto iter = frontiers_.begin(); iter != frontiers_.end();) {
    if (haveOverlap(iter->box_min_, iter->box_max_, update_min, update_max) &&
        isFrontierChanged(*iter)) { // 如果与box有重叠并且..，则删除该边界点
  ...
  Vector3d search_min = update_min - Vector3d(1, 1, 0);
  Vector3d search_max = update_max + Vector3d(1, 1, 0);
  search_min[2] = search_max[2] = 0.3;
  ...
  for (int x = min_id(0); x <= max_id(0); ++x)
    for (int y = min_id(1); y <= max_id(1); ++y)
      for (int z = min_id(2); z <= max_id(2); ++z) {
        ...
        auto map_iter_ = frontier_flag_.find(toadr(cur));
        int cur_index_in_frontier_flag_ = toadr(cur);
        if(map_iter_ == frontier_flag_.end()) {
          frontier_flag_[cur_index_in_frontier_flag_] = 0;
        }
        if (frontier_flag_[cur_index_in_frontier_flag_] == 0 && knownfree(cur) && isNeighborUnknown(cur)) {
        // if (frontier_flag_[cur_index_in_frontier_flag_] == 0 && knownoccupied(cur) && isNeighborUnknown(cur)) {
          // Expand from the seed cell to find a complete frontier cluster
          // std::cout << "expandFrontier " << x << " " << y << " " << z << std::endl;
          expandFrontier(cur);
          // std::cout << "finish expending curent frontier" << std::endl;

          // debug
          // vector<Eigen::Vector3d> cur_frontiers_;
          // Eigen::Vector3d cur_postion_;
          // edt_env_->sdf_map_->indexToPos(cur, cur_postion_);
          // cur_frontiers_.push_back(cur_postion_);
          // Frontier frontier;
          // frontier.cells_ = cur_frontiers_;
          // debug_frontiers_.push_back(frontier);
        }
      }

  // debug
  // std::cout << "size of tmp_frontiers_: " << tmp_frontiers_.size() << std::endl;
  // for(auto cur = tmp_frontiers_.begin(); cur != tmp_frontiers_.end(); cur++){
  //   debug_frontiers_.push_back(*cur);
  // }
  std::cout << "finish expending frontiers" << std::endl;
{
  std::cout << "size of tmp_frontiers: " << tmp_frontiers_.size() << std::endl;
  for (auto it = tmp_frontiers_.begin(); it != tmp_frontiers_.end(); ++it) {
    std::cout<<cv::Point2d((*it).cells_.size(),(*it).filtered_cells_.size())<<" ";
  }
  std::cout<<std::endl;
}
  splitLargeFrontiers(tmp_frontiers_);
{
  std::cout << "size of frontiers: " << tmp_frontiers_.size() << std::endl;
  for (auto it = tmp_frontiers_.begin(); it != tmp_frontiers_.end(); ++it) {
    std::cout<<cv::Point2d((*it).cells_.size(),(*it).filtered_cells_.size())<<" ";
  }
  std::cout<<std::endl;
  ...
  std::cout << "FrontierFinder end-------------------------" << std::endl;
}
...
void FrontierFinder::expandFrontier(
  ...
  // std::cout << "expandFrontier step 0" << std::endl;
  ...
  // Search frontier cluster based on region growing (distance clustering)  using bfs
  // std::cout << "expandFrontier step 1" << std::endl;
  // int tmp_count = 0; //debug
  while (!cell_queue.empty()) {

    // std::cout << "expandFrontier step 1.1 " << tmp_count << std::endl;

    auto cur = cell_queue.front();
    cell_queue.pop();
    // auto nbrs = allNeighbors(cur); // 26 neighbors
    auto nbrs = allNeighbors2D(cur); // 8 neighbors

    // std::cout << "expandFrontier step 1.2 " << tmp_count << std::endl;

    for (auto nbr : nbrs) {
      // Qualified cell should be inside bounding box and frontier cell not clustered
      // std::cout << "start judge nbr" << std::endl;

      int adr = toadr(nbr);
      // std::cout << "1.2.0" << std::endl;
      auto map_iter_ = frontier_flag_.find(adr);
      if(map_iter_ == frontier_flag_.end()) {
      ...
      // std::cout << "nbr id, pos, flag: " << nbr.transpose() << " " << nbr_pos.transpose() << " " << frontier_flag_[adr] << std::endl;

      if (frontier_flag_[adr] == 1 || !edt_env_->sdf_map_->isInBox(nbr) ||
          !(knownfree(nbr) && isNeighborUnknown(nbr)))
        continue;

      // std::cout << "translate index of nbr to position" << std::endl;
      edt_env_->sdf_map_->indexToPos(nbr, pos);
      if (pos[2] < 0.11) continue;  // Remove noise close to ground
      // std::cout << "expanded push back" << std::endl;
      expanded.push_back(pos);
      // std::cout << "cell_queue push" << std::endl;
      cell_queue.push(nbr);
      // std::cout << "finish push" << std::endl;
      frontier_flag_[adr] = 1;
      // std::cout << "finish judge nbr" << std::endl;
    }

    // std::cout << "size of expanded" << expanded.size() << std::endl;
    // std::cout << "size of cell_queue" << cell_queue.size() << std::endl;
    // std::cout << "expandFrontier step 1.3 " << tmp_count << std::endl;

    // tmp_count++; //debug
  }

  // std::cout << "expandFrontier step 2" << std::endl;
  // std::cout << "size of expanded "<< expanded.size() << std::endl;
  ...
  // std::cout << "expandFrontier step 3" << std::endl;
}
...
void FrontierFinder::splitLargeFrontiers(list<Frontier>& frontiers) {
  ...
  for (auto it = frontiers.begin(); it != frontiers.end(); ++it) {
    ...
    if (splitHorizontally(*it, splits)) {
      ...
      // std::cout<<"splitsx "<<splits.size()<<std::endl;
      // std::cout<<"size of tmps "<<tmps.size()<<std::endl;
      splits.clear();
    } else{
      tmps.push_back(*it);
      // std::cout<<"splitsy "<<1<<std::endl;
    }
  }
  frontiers = tmps;
}
...
bool FrontierFinder::splitHorizontally(const Frontier& frontier, list<Frontier>& splits) {
  ...
  // std::cout << "size of curent frontier " << frontier.filtered_cells_.size() << std::endl;
  ...
  if (!need_split) {
    // std::cout << "need not split" << std::endl;
    return false;
  }
  ...
  // std::cout << "size of ftr1.cells_(before downsampling) " << ftr1.cells_.size() << std::endl;
  // std::cout << "size of ftr2.cells_(before downsampling) " << ftr2.cells_.size() << std::endl;

  computeFrontierInfo(ftr1);
  computeFrontierInfo(ftr2);

  // std::cout << "size of ftr1.cells_(after downsampling) " << ftr1.filtered_cells_.size() << std::endl;
  // std::cout << "size of ftr2.cells_(after downsampling) " << ftr2.filtered_cells_.size() << std::endl;
...
void FrontierFinder::updateFrontierCostMatrix() {
  ...
  {
    std::cout << "size of frontiers: " << frontiers_.size() << std::endl;
...
bool FrontierFinder::isFrontierChanged(const Frontier& ft) {
  for (auto cell : ft.cells_) {
    Eigen::Vector3i idx;
    edt_env_->sdf_map_->posToIndex(cell, idx);
    if (!(knownfree(idx) && isNeighborUnknown(idx))) return true; // 如果不是free区域或者距离未知区域较近，则认为该点是变化
    // if (!(knownoccupied(idx) && isNeighborUnknown(idx))) return true; // 如果不是free区域或者距离未知区域较近，则认为该点是变化
  }
  return false;
}
...
void FrontierFinder::computeFrontierInfo(Frontier& ftr) {
  ...
  ftr.average_.z() = 0.3;  // camera height
...
void FrontierFinder::computeFrontiersToVisit() {
  ...
  for (auto& tmp_ftr : tmp_frontiers_) {
    ...
    // std::cout << "size of tmp_ftr.viewpoints_" << tmp_ftr.viewpoints_.size() << std::endl;
...
void FrontierFinder::getDebugFrontiers(vector<vector<Eigen::Vector3d>>& clusters) {
  clusters.clear();
...
void FrontierFinder::getDebugSamplePoints(vector<vector<Eigen::Vector3d>>& clusters){
  clusters = debugSamplePoints;
}
...
void FrontierFinder::sampleViewpoints(Frontier& frontier) {
  ...
  // vector<Eigen::Vector3d> debugSamplePoint;
  ...
  for (double rc = candidate_rmin_, dr = (candidate_rmax_ - candidate_rmin_) / candidate_rnum_;
    ...
    for (double phi = -M_PI; phi < M_PI; phi += candidate_dphi_) {
      ...
      // std::cout<<"sample_pos "<<sample_pos.transpose()<<"visib_num "<<visib_num<<std::endl;
      if(visib_num == 0){
        // debugSamplePoint.push_back(sample_pos);
      }
    ...
    // debugSamplePoints.push_back(debugSamplePoint);
}
  ...
  auto checkChanges = [&](const list<Frontier>& frontiers) {
    for (auto ftr : frontiers) {
      ...
      for (auto cell : ftr.cells_) {
        ...
        // if (!(knownoccupied(idx) && isNeighborUnknown(idx)) && ++change_num >= change_thresh)
          return true;
      }
    }
    return false;
  ...
  for (int x = -vox_num; x <= vox_num; ++x)
    for (int y = -vox_num; y <= vox_num; ++y)
      for (int z = -1; z <= 1; ++z) {
        ...
        if (edt_env_->sdf_map_->getOccupancy(vox) == SDFMap::UNKNOWN || edt_env_->sdf_map_->getOccupancy(vox) == SDFMap::OCCUPIED) return true;
      }
  return false;
}
...
int FrontierFinder::countVisibleCells(
  ...
  // std::cout<<"pos "<<pos.transpose()<<" yaw"<<yaw<<std::endl;
  int visib_num = 0;
  Eigen::Vector3i idx;
  for (auto cell : cluster) {
    // Check if frontier cell is inside FOV
    if (!percep_utils_->insideFOV(cell)){
      // std::cout<<cell.transpose()<<" not insideFOV"<<std::endl;
      continue;
    } 
    ...
    while (raycaster_->nextId(idx)) {
      if (edt_env_->sdf_map_->getInflateOccupancy(idx) == 1 ||
        ...
        edt_env_->sdf_map_->indexToPos(idx, pos_err);
        // std::cout<<cell.transpose()<<" "<<pos_err.transpose()<<" not visibile"<<std::endl;
        break;
      }else{
        Eigen::Vector3d pos_err ;
        edt_env_->sdf_map_->indexToPos(idx, pos_err);
        // std::cout<<cell.transpose()<<" "<<pos_err.transpose()<<"  visibile"<<std::endl;
      }
    }
    if (visib){
      visib_num += 1;
    } else{
      // std::cout<<cell.transpose()<<" not visibile"<<std::endl;
    }
  }
  return visib_num;
}
...
void FrontierFinder::downsample(
  ...
  const double leaf_size = edt_env_->sdf_map_->getResolution() * down_sample_;  // 0.2 * 3
...
inline vector<Eigen::Vector3i> FrontierFinder::fourNeighbors2D(const Eigen::Vector3i& voxel) {
...
// return indexes of all neighbors
...
// return indexes of all neighbors 2D
inline vector<Eigen::Vector3i> FrontierFinder::allNeighbors2D(const Eigen::Vector3i& voxel) {
...
inline bool FrontierFinder::isNeighborUnknown(const Eigen::Vector3i& voxel) {
  // At least one neighbor is unknown   input is id
  auto nbrs = sixNeighbors(voxel);
  // auto nbrs = fourNeighbors2D(voxel);
...
inline bool FrontierFinder::inmap(const Eigen::Vector3i& idx) {
  ...
  return edt_env_->sdf_map_->isInBox(idx);
}
