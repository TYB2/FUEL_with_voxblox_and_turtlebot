...
std_msgs::ColorRGBA IDToColor(int id,int max_id,int min_id){
    ...
    double h = 4.0 * id / max_id;
    double x = 1.0 * (1 - abs(fmod(h, 2.0) - 1));
...
void PlanningVisualization::drawCubes(vector<Eigen::Vector3d> list, const double& scale,
                                      const Eigen::Vector4d& color, const string& ns, const int& id,
                                      const int& pub_id) {
  // if(pub_id == 4){
...
void PlanningVisualization::displayLineList2(const vector<Eigen::Vector3d>& list1,double line_width,
                                            const Eigen::Vector4d& color, int id, int pub_id) {
  visualization_msgs::Marker mk;
  mk.header.frame_id = "world";
  mk.header.stamp = ros::Time::now();
...
void PlanningVisualization::drawGoal(Eigen::Vector3d goal, double resolution,
  ...
  displaySphereList(goal_vec, resolution, color, GOAL + id % 100, 6);
}
