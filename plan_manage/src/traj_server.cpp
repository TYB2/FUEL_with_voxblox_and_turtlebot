...
ros::Publisher trajectory_pub, turtlebot_trajectory_pub;
nav_msgs::Odometry odom;
shared_ptr<actionlib::SimpleActionClient<move_base_msgs::MoveBaseAction>> action_client; // 
...
void cmdCallback(const ros::TimerEvent& e) {
  ...
  actionlib::SimpleClientGoalState ac_status_tmp = action_client->getState();
  if(ac_status_tmp == actionlib::SimpleClientGoalState::ACTIVE ||
      ac_status_tmp == actionlib::SimpleClientGoalState::PENDING){
      return;
  }
  ...
  if(wait_flag){
    ...
    if (t_cur < traj_duration_ && t_cur >= 0.0) {
      ...
      cout << "acc: " << acc.transpose() << " yawdot: " << yawdot << " pos: " << pos.z() <<endl;
  ...
  if(goal_flag){
    ...
    // samples_array.header.seq = tmp_seq++;
    // samples_array.header.stamp = time_now;
    // samples_array.header.frame_id = "world";
    // samples_array.points.clear();
    // trajectory_point.position_W.x() = pos(0);
    // trajectory_point.position_W.y() = pos(1);
    // trajectory_point.position_W.z() = pos(2);  
    // trajectory_point.velocity_W.x() = vel(0);
    // trajectory_point.velocity_W.y() = vel(1);
    // trajectory_point.velocity_W.z() = vel(2);
    // trajectory_point.acceleration_W.x() = acc(0);
    // trajectory_point.acceleration_W.y() = acc(1);
    // trajectory_point.acceleration_W.z() = acc(2);
    ...
    tf::Quaternion quat = tf::Quaternion(tf::Vector3(0.0, 0.0, 1.0), yaw);
    next_goal.pose.orientation.x = quat.getX();
    next_goal.pose.orientation.y = quat.getY();
    next_goal.pose.orientation.z = quat.getZ();
    next_goal.pose.orientation.w = quat.getW();
    next_goal.pose.position.x = pos(0);
    next_goal.pose.position.y = pos(1);
    next_goal.pose.position.z = pos(2);
    move_base_msgs::MoveBaseGoal goal;
    goal.target_pose = next_goal;
    action_client->sendGoal(goal);

    // if(pos(2) != 0.2) {
    //   ROS_ERROR("z of position is not 0.2");
    // }
...
int main(int argc, char** argv) {
  ...
  trajectory_pub = node.advertise<trajectory_msgs::MultiDOFJointTrajectory>("/firefly/command/trajectory", 5);
  // turtlebot_trajectory_pub = node.advertise<nav_msgs::Path>("/my_turtlebot/path", 5);
  actionlib::SimpleActionClient<move_base_msgs::MoveBaseAction> action_client_1("/move_base"); //
  action_client.reset(new actionlib::SimpleActionClient<move_base_msgs::MoveBaseAction>("/move_base")); 
